...
This document provides an overview of the Lerncoaching Gesprächsführer project for developers and AI agents.

## Project Overview

A German-language learning coaching conversation guide for educators. Single-page app with no backend — all state is in-memory. Built with TanStack Start and deployed on Netlify.

## Key Files

- `src/routes/index.tsx` — entire coaching app (all components, data, hooks)
- `src/routes/__root.tsx` — HTML shell with Google Fonts (Fraunces + DM Sans)
- `src/styles.css` — all custom CSS; CSS variables define the warm green/cream palette

## Coaching Data

Phases are defined as the `PHASES` constant in `index.tsx`. Each phase has `id`, `label`, `title`, `desc`, and `questions[]`. Questions support inline `<strong>` HTML. To add/edit phases or questions, edit this array directly.

## State Model

- Students: `string[]` in `useState` — names only, no persistence
- Notes: `Record<string, string>` keyed as `${studentName}__${phaseId}`
- Highlights: `Set<string>` keyed as `${phaseId}__${questionIndex}`
- Timer: custom `useTimer` hook using `setInterval`

## CSS Conventions

All layout classes are hand-crafted (`.card`, `.phase-step`, `.question-item`, etc.). Tailwind utilities are not used inline in JSX for this project. Color tokens live in `:root` as CSS custom properties (`--forest`, `--cream`, `--ink-muted`, etc.).

## No Build Artifacts Needed

No API routes, no database, no Netlify Functions. The scaffold's `src/data/products.ts` and `src/routes/products/` directory are unused and can be deleted if desired.
